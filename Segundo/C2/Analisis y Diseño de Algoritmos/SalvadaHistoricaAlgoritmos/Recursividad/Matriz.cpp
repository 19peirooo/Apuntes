...
/*  Calculadora Determinantes
    Calcula el determinante de un matriz cuadrada
    Precondicion: La Matriz no puede ser nula y ha de ser cuadrada
    Complejidad Temporal: T(n)= n*T(n-1) * O(n^2) ,O(n!)
    Complejidad Espacial: M(n) = n + n^2 = O(n^2)
*/

Matriz* Matriz :: calcularAdjunto(int fila, int columna) {
  Matriz* adjunto = new Matriz(this->n_filas-1,this->n_columnas-1);
  int filaAdj = 0;
  int colAdj = 0;
  for (int i = 0; i < this->n_filas; i++) {
    colAdj = 0;
    if (i != fila) {
      for (int j = 0; j < this->n_columnas; j++) {
        if (j != columna) {
          adjunto->matriz[filaAdj][colAdj] = this->matriz[i][j];
          colAdj++;
        }
      }
      filaAdj++;
...
double Matriz::calcularDeterminante() {
  ...
  } else if (this->n_filas == 2) {
    return (this->matriz[0][0]*this->matriz[1][1])-(this->matriz[0][1]*this->matriz[1][0]); 
  } 

  for (int i = 0; i < this->n_filas; i++) {
    Matriz* adjunto = this->calcularAdjunto(i,0);
    if (i % 2 == 0) {
      signo = 1;
    } else {
      signo = -1;
    }
    det += signo*this->matriz[i][0]*adjunto->calcularDeterminante();
  }
  return det;
}