...
Dart es un lenguaje de programación moderno desarrollado por Google para Flutter. Características principales:

- Lenguaje orientado a objetos con sintaxis tipo C
- Soporte para programación asíncrona con Futures
- Sistema de tipos robusto con null safety
- Compilación JIT (desarrollo) y AOT (producción)
- Colecciones flexibles (List, Set, Map)
- Constructores versátiles y herencia de clases

Flutter se base en un lenguaje de programación llamado Dart desarrollado por Google. Se trata de un lenguaje de ejecución y codificación rápida, que mezcla características de muchos de los lenguajes de programación existentes a día de hoy. Podemos probar todas las funcionalidades desde el IDE que se comentó en el punto anterior o directamente en un compilador online disponible en [https://dartpad.dev](https://dartpad.dev/)

Antes de empezar con la sintaxis del lenguaje, comentar que al igual que otros lenguajes de programación, flutter cuenta con un método main sobre el cual comienza todas las ejecuciones
...
En dar no es necesario el tipado de una variable si se utiliza la palabra reservada var, ya que así dart decide cual es el tipo de dicha variable
...
Las funciones representan las parte de código reutilizables que son ejecutadas con la llamada de la misma. Para poder definir una función se utiliza la siguiente sintaxis:
...
Para poder crear una lista, se puede iniciar con un tamaño vacío o con una cantidad de elementos predefinido

```java
List<Examen> lista = [];
// en el caso de querer inicializarla con datos ya metidos se utiliza la siguiente sintaxis
List<Examen> lista = [
    new Examen("Inicial", "Desarrollo móvil", 1, 3),
    new Examen("Final", "Desarrollo móvil", 3, 3),
  ];
```
...
En el caso de querer crear una lista de diferentes tipos de datos, la tipariamos como dynamic
...
Si quieres crear una lista de tamaño fijo, se utiliza el método filled 

`List<Examen?> listaFija = List.filled(5,null);`

</aside>

Si queremos agregar un elemento utilizaríamos el método add

```java
lista.add(new Examen("Intermedio", "Desarrollo móvil", 3, 3));
```

Si queremos eliminar un elemento de la lista, ejecutamos el método remove o removeAt, dependiendo si indico un elemento o una posición
...
Si queremos capturar un elemento de la lista, utilizamos el método get

```java
  var busqueda = lista[0];
...
Y también podríamos utilizar un foreach

```java
lista.forEach((Examen item) {
    item.mostrarDatos();
  });
```

En el caso de querer hacer búsquedas, se podrían hacer de diferentes formas

```java
lista.contains()
lista.indexOf()
// retorna el primero que cumple la condición de búsqueda
var resultado = lista.firstWhere(
		(elemento) => elemento.asignatura == 'Desarrollo movil');
// retorna todos los que cumple la condición de búsqueda
var resultado = lista.any(
    (elemento) => elemento.asignatura == 'Desarrollo movil',
  );
```
...
![](https://www.notion.so./images/clases_mixins.png)
