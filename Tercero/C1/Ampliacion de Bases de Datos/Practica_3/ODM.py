...
class Model:
    ...
    def __setattr__(self, name: str, value: str | dict) -> None:
        ...
        if name in {'_modified_vars', '_required_vars', '_admissible_vars', '_db', '_data', '_location_var','_unique_indexes','_cache'}:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        en el objeto 
        """
        if name in {'_modified_vars', '_required_vars', '_admissible_vars', '_db', '_data', '_location_var','_unique_indexes'}:
        ...
        #Si ya existe, actualizo solo los atributos que he modificado
            ...
            for name,updated in self._modified_vars.items():
                if updated:
                    ...
                    #Eliminar de cache
                    redis_id = f"{self.__class__.__name__}:{self._data['_id']}"
                    self._cache.delete(redis_id)
    ...
    def find_by_id(cls, id: str) -> Self | None:
        ...
        redis_id = f"{cls.__name__}:{id}"

        # Si devuelve algo es que esta en cache
        cache_data = cls._cache.get(redis_id)
        ttl = 24*60*60 # 24h

        if cache_data is not None:
            cls._cache.expire(redis_id,ttl)
            doc = json.loads(cache_data)
            print("Obteniendo datos de cache...")
            return doc

        print("No se ha encontrado en cache... Buscando en Mongo...")
        mongo_data = cls._db.find_one({"_id":ObjectId(id)})
        if mongo_data is not None:
            mongo_data['_id'] = str(mongo_data['_id'])
            cls._cache.set(redis_id,json.dumps(mongo_data))
            cls._cache.expire(redis_id,ttl)
            return mongo_data
    ...
    def init_class(cls, db_collection: pymongo.collection.Collection, indexes:dict[str,str], required_vars: set[str], admissible_vars: set[str], cache: redis.Redis) -> None:
        ...
        for idx_field in indexes.get('regular_indexes',[]):
            cls._db.create_index([idx_field,pymongo.TEXT])
...
def initApp(definitions_path: str = "./models.yml", mongodb_uri="mongodb://localhost:27017/", db_name="db_Practica3", scope=globals()) -> None:
    """ 
    ...
    redis_cache.set("maxmemory","150mb")
    redis_cache.set("maxmemory-policy","allkeys-lru")
...
def dropDataBase(mongodb_uri="mongodb://localhost:27017/", db_name="db_Practica3", scope=globals()) -> None:
    client = pymongo.MongoClient(mongodb_uri) #Se conecta con MongoDB
